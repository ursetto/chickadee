  ...
  (define (path->uri-string p)
    (string-append
     ...
     (string-intersperse (map (lambda (x)
                              ...
                              p) "/")))
  ;; We generating a relative path from the current path. We use the
  ;; current request uri. This won't be available if generating pages statically,
  ;; so we could just pass the current page's expected URI into this function.
  ;; It's a bit ugly since we also have to strip out the host, port, scheme etc.
  (let ((u1 (uri-reference (path->uri-string p)))
        (u2 (update-uri (uri-reference "")   ; strip all but path
                        path: (uri-path (request-uri (current-request))))))
    ;; FIXME: This doesn't work anyway. We are rewriting (with rewrite-uri) the incoming
    ;; request to point to /cdoc/?q=xxxx, and then restarting the request. Thus we
    ;; must calculate the current page URI and pass it to this function to do the
    ;; relative compare. 
    ;; FIXME: That doesn't work either. /cdoc/?path=9p is technically accessible
    ;; and will not redirect (so we can't assume the final path is /doc/9p), and
    ;; /cdoc/ match page is visible to users. This is all caused by the dubious
    ;; implementation of internally rewriting the /doc URI to /cdoc without saving
    ;; the original URI. So we either need to 1) save the request path on entry to the
    ;; vhost map, skipping if it's already set; or 2) stop rewriting the uri and
    ;; handle chickadee requests as-is.
...
(define (uri-relative-from u1 u2)
  ...
  (let ((rel (uri-common:uri-relative-from (add-base u1) (add-base u2))))
    ...
    (match (uri-path rel)
           ...
           (() rel)
           ;; . FAILS on files
           ;; uri-generic bug: /foo/bar to /foo/bar should be ./bar (like /foo/baz to /foo/bar is ./bar) but is ""
           ;; Also /foo/bar/ to /foo/bar/ is "" but should be "." or "./"
           ;; /foo/bar/baz to /foo/bar/ is "./", could be "."
           (() (uri-reference "."))  ;; equal paths yield empty relative path. . so directories work
           ;; "./" == (uri->string (uri-relative-from (uri-reference "/base/foo/") (uri-reference "/base/foo/x")))
           (("." "") rel)   ;; ./ to /chickadee/ from /chickadee/x
           ;; plain (".") won't be generated by uri-generic
           (("." . rest) (update-uri rel path: rest))   ;; drop leading . if later paths
           ;; (('/ "_xyzzy_" . path) ('/ . ,path))
